...
function Categories ({apps, categoryId, category}) {
    ...
    // ==============checkboxes hanlder============
    const headCheckHandler = () => { //check all individual app
        const isChecked = $('#head__checkbox').prop('checked')
        if(isChecked) {
            $('input[name="body__checkbox"]').prop('checked', true)
        } else (
            $('input[name="body__checkbox"]').prop('checked', false)
        )
        appDeleteBtn()
    }

    const bodyCheckHandler = (appId) => { //check the all-drivers checkbox
        const isCheckedAll = $('input[name="body__checkbox"]').length === $('input[name="body__checkbox"]:checked').length
        if(isCheckedAll) {
    ...
    const deleteAppHandler = () => {
        if($('input[name="body__checkbox"]:checked').length === $('input[name="body__checkbox"]').length) {
            sortedAppsIds.map(id => {
                axios.delete(`http://localhost:1337/api/apps/${id}`, {
                    headers: {
                    ...
                    .then(respond => {
                        apps.map(app => {
                            if(app.id === respond.data.data.id) {
                                axios.delete(`http://localhost:1337/api/upload/files/${app.attributes.logo.data.id}`, {
                                    headers: {
                                        'Authorization': `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}` 
                                    }
                                })
        ...
        } else if ($('input[name="body__checkbox"]:checked').length < $('input[name="body__checkbox"]').length && $('input[name="body__checkbox"]:checked').length > 0) {
            appIds.map(id => {
                axios.delete(`http://localhost:1337/api/apps/${id}`, {
                    headers: {
                        'Authorization': `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}` 
            ...
            <div className={cx('apps-container')}>
                <Table striped bordered hover>
                    ...
                    <tbody className={cx('body')}>
                        {apps.map((app) => {
                            if(app.attributes.category.data && app.attributes.category.data.attributes.name === category.attributes.name) {
                                sortedApps.push(app)
                            }
                            if(category.attributes.name === "Tất cả") {
                                sortedApps.push(app)
