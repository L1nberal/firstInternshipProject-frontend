...
function Comments({ appId, users }) {
    ...
    const deleteCommentHandler = async (commentId) => {
        await axios
            ...
            .then((respond) => {
                setCommentsToShow(
                    commentsToShow.filter((commentFiltered) => {
    ...
    const showUpdateSection = (commentId) => {
        ...
        for (let i = 0; i < $('div[name="each-comment__update-section"]').length; i++) {
            if (
                $('div[name="each-comment__update-section"]')[i].id !=
                $(`#each-comment__update-section-${commentId}`)[0].id
            ) {
    ...
    const showReplyCommentUpdateSection = (commentId) => {
        ...
        for (let i = 0; i < $('div[name="each-reply-comment__update-section"]').length; i++) {
            if (
                $('div[name="each-reply-comment__update-section"]')[i].id !=
                $(`#each-reply-comment__update-section-${commentId}`)[0].id
            ) {
                $('div[name="each-reply-comment__update-section"]')[i].style.display = 'none';
            }
        }
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Footer>
                    <Button onClick={handleClose} className={cx('modal-btn')} variant="secondary">
                        Đã hiểu
                    </Button>
                                ...
                                <React.Fragment key={index}>
                                    <div className={cx('comments')}>
                                        <Toast className={cx('each-comment')}>
                                            <Toast.Header closeButton={false} className={cx('each-comment__header')}>
                                            ...
                                            <Toast.Body className={cx('each-comment__content')}>
                                            ...
                                            if (replyComment.attributes.parent_Id === comment.id) {
                                                return (
                                                    <Toast className={cx('each-reply-comment')} key={replyComment.id}>
                                                        ...
                                                        <Toast.Body className={cx('each-reply-comment__content')}>
                                                            ...
                                                            {user != null && (
                                                                <React.Fragment>
                                                                    ...
                                                                    {(user.id === replyComment.attributes.userId ||
                                                                        ...
                                                                        <button
                                                                            onClick={() =>
                                                                                deleteCommentHandler(replyComment.id)
                                                                            }
                                                                            className={cx(
                                                                                'each-reply-comment__delete-btn',
                                                                            )}
                                                                        >
                                                                            delete
                                                                        </button>
                                                                    ...
                                                                    {user.id === replyComment.attributes.userId && (
                                                                        <button
                                                                            onClick={() =>
                                                                                showReplyCommentUpdateSection(
                                                                                    replyComment.id,
                                                                                )
                                                                            }
                                                                            className={cx(
                                                                                'each-reply-comment__update-btn',
                                                                            )}
                                                                        >
                                                                            update
                                                                        </button>
                                                                    ...
                                                                    <button
                                                                        onClick={() =>
                                                                            showReplyHandler(replyComment, comment.id)
                                                                        }
