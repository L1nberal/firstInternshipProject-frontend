...
function OrganisationUpdate({ organisationId, organisations, apps, organisation }) {
        ...
        if (error === false) {
            ...
            async function submit() {
                ...
                if (logo === null && photos.length === 0) {
                    axios
                        .put(
                            `http://localhost:1337/api/organisations/${organisationId}`,
                            {
                                data: {
                                    name: name,
                        ...
                        .then(async (respond) => {
                            await organisations.map((organisationMapped) => {
                                if (organisationMapped.id === organisationId) {
                                    axios
                                        .delete(
                                            `http://localhost:1337/api/upload/files/${organisationMapped.attributes.logo.data.id}`,
                                            {
                                                headers: {
                                                    Authorization: `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}`,
                                                },
