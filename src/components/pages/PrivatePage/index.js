...
function PrivatePage({ userId, userInfor, apps, users }) {
    ...
    const formSchema = Yup.object().shape({
        ...
        newPassword: Yup.string().required('Bạn chưa nhập mật khẩu mới').min(6, 'mật khẩu phải dài ít nhất 6 kí tự'),
    ...
    const userDeleteHandler = async (userId) => {
                    ...
                    if (comment.attributes.userId === userId) {
                        respond.data.data.map(async (commentMapped) => {
                            if (commentMapped.attributes.parent_Id === comment.id) {
                                await axios
                                    .delete(`http://localhost:1337/api/comments/${commentMapped.id}`, {
                                        headers: {
                                            Authorization: `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}`,
                                        },
                                    })
                        ...
                        axios
                            .delete(`http://localhost:1337/api/comments/${comment.id}`, {
                                headers: {
                                    Authorization: `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}`,
                                },
                            })
        ...
        // ===========delete user==================
        await axios
            .delete(`http://localhost:1337/api/users/${userId}`, {
                headers: {
                    Authorization: `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}`,
                },
            })
            .then(async (respond) => {
                await users.map((user) => {
                    if (user.id === userId) {
                        // =============delete avatar===============
                        axios
                            .delete(`http://localhost:1337/api/upload/files/${user.avatar.id}`, {
                                headers: {
                                    Authorization: `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}`,
                                },
                            })
    ...
    const disableUserHandler = async (userId) => {
        ...
        await axios
            ...
            .then((respond) => {
                // =============delete avatar===============
                axios
                    .delete(`http://localhost:1337/api/upload/files/${userInfor.avatar.id}`, {
                        headers: {
                            Authorization: `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}`,
                        },
                    })
        ...
        return (
            <div className={cx('wrapper')}>
                ...
                >
                    ...
                    >
                        {update === true ? (
                            <div className={cx('update-section')}>
                                ...
                                >
                                    Trở về
                                    <FontAwesomeIcon icon={icons.faRightToBracket} className={cx('icon')} />
                                </Button>
                                {/* ====================infor update form================== */}
                                    ...
                                    >
                                        <div className={cx('form__password')}>
                                            ...
                                            <div>
                                                ...
                                                Show Password
                                                <div className={cx('invalid-feedback')}>
                                                    {errors.confirmNewPasword?.message}
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className={cx('private-infor')}>
                                ...
                                <div className={cx('infor-container')}>
                                    <div>
                                        Tên người dùng: <span>{userInfor.username}</span>
                                    </div>
                                    <div>
                                        Email: <span>{userInfor.email}</span>
                                    </div>
                                    <div>
                                        Vai trò:{' '}
                                        {userInfor.isAdmin ? <span>Admin</span> : <span>Người dùng thông thường</span>}
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    {userInfor.isAdmin && (
                        <Tab eventKey="apps" title="Ứng dụng đã đăng" className={cx('posted-apps-container', 'tab')}>
                            <div className={cx('apps-posted')}>
                                {postedApps.map((app) => {
                                    return (
                                        <Card key={app.id} border="success" className={cx('each-app')}>
                                            <Card.Header className={cx('each-app__name')}>
                                                {app.attributes.name}
                                            </Card.Header>
                    ...
                    {userInfor.isAdmin && (
                        <Tab eventKey="users" title="Các user khác" className={cx('other-users-container', 'tab')}>
                            <div className={cx('users')}>
                                {users.map((user) => {
                                    if (user.id != userInfor.id) {
                                        return (
                                            <Card key={user.id} border="secondary" className={cx('each-user')}>
