...
function AppUpdate(
    ...
    const { user } = UserAuth()
    // ===========set category choice============
    const [category, setCategory] = useState(app.attributes.category.data ? app.attributes.category.data.attributes.name : "Chọn phân loại")
                        ...
                        .then(async (response) => {
                            await apps.map(appMapped => {
                                if(appMapped.id === appId) {
                                    axios.delete(`http://localhost:1337/api/upload/files/${appMapped.attributes.logo.data.id}`, {
                                        headers: {
                                            'Authorization': `Bearer ${process.env.REACT_APP_FULL_ACCESS_TOKEN}` 
                                        }
                                    })
                ...
                <Form className={cx('form')} onSubmit={e => submitHandler(e)}>
                    ...
                    <Form.Group className={cx('mb-3', 'formBasic', 'categories-dropdown')} controlId="formBasic">
                        ...
                        <Dropdown as={ButtonGroup}>
                            <Button
                                variant="secondary"
                                className={cx('categories-dropdown__btn')}
                            >
                                {category}
                            </Button>
